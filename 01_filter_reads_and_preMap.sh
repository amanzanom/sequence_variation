...
echo "  -> Started filtering reads in " $1 " and " $2 " - "$(date +"%D %T");

fastx_artifacts_filter -Q33 -v -i $1 | fastq_quality_trimmer -Q33 -t 28 -l 50 -v > side_1.fastq;
fastx_artifacts_filter -Q33 -v -i $2 | fastq_quality_trimmer -Q33 -t 28 -l 50 -v > side_2.fastq;

echo "  -> Finished filtering reads in " $1 " and " $2 " - "$(date +"%D %T");
echo "";
...
mkdir BWT_INDEX;
cd BWT_INDEX;
bowtie2-build --quiet $3 backbone_pre;
cd ..;
#echo "  -> Started mapping SE reads with bowtie - "$(date +"%D %T");
#bowtie2 -q --phred33 --very-sensitive-local --no-unal -p $4 --reorder -x BWT_INDEX/backbone_pre -U readsGood_SE.fastq -S readsGood_SE_vs_backbone_pre.sam;
#echo "  -> Finished mapping SE reads with bowtie - "$(date +"%D %T");
#echo "";
echo "  -> Started mapping PE reads with bowtie - "$(date +"%D %T");
bowtie2 -q --phred33 --very-sensitive-local -I 100 -X 600 --fr --no-mixed --no-discordant --no-dovetail --no-unal -p $4 --reorder -x BWT_INDEX/backbone_pre -1 readsGood_1.fastq -2 readsGood_2.fastq -S readsGood_PE_vs_backbone_pre.sam;
echo "  -> Finished mapping PE reads with bowtie - "$(date +"%D %T");
echo "";
#echo "  -> Started mapping MIXED reads with bowtie - "$(date +"%D %T");
#bowtie2 -q --phred33 --very-sensitive-local --no-discordant --no-dovetail --no-unal -p $4 --reorder -x BWT_INDEX/backbone_pre -U 'readsGood_SE.fastq,readsGood_1.fastq,readsGood_2.fastq' -S readsGood_MIX_vs_backbone_pre.sam;
