...
while ($line=<INSAM>){
	...
	if ($line=~ m/^\S+\s+(\S+)\s+(\S+)\s+(\S+)\s+\S+\s+(\S+)\s+\S+\s+\S+\s+\S+\s+(\S+)/){
		...
		if ( ($alnLength/length($sequence))*100 >= $minOverlap || (($alnLength/length($sequence))*100 < $minOverlap && ( ($start - $skippedLeft <= 0 && $skippedRight<=4)  || ($start+$skippedLeft+$alnLength+$skippedRight >= $backboneLength{$id} && $skippedLeft<=4) ) )){
			print OUTSAM $line;
		}
	}
}
